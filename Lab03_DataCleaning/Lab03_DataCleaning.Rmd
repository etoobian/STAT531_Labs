...
```{r}
bids_new <- select(.data=mutate(.data=bids, PRICE_clean=PRICE), -PRICE)
...
filter(.data=bids, as.numeric(PRICE) < 0)$PRICE
filter(.data=bids, as.numeric(PRICE) == -999)$PRICE
filter(.data=bids_new, PRICE_clean <= 0.0001, PRICE_clean >= 0, BID_WON=="TRUE" | BID_WON=="true")
bids$PRICE[idx]
rm(idx)
```
...
It is uncommon for there to be CPM values greater than 50 or less than 0.0001, but it is not implausible. What had been done prior to this section include removing irretrievable data values (-999). We can validate reverse negation using histograms representing the distribution of newly modified PRICE_clean.
...
| PRICE | Includes non-numeric characters, negated values, non-digit characters (excludind '.'), negative sentinels | Validity, Completeness | Reverse negate applicable values, drop sentinels, convert to type numeric, drop extreme values |
