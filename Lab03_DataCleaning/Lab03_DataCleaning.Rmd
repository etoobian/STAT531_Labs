...
1.  In **2–4 sentences**, describe **what each row** in this dataset represents.
  There is no consistent unique identifier that can be found for any of the rows. Ideally it would be encompassed entirely by AUCTION_ID, PUBLISHER_ID and RESPONSE_TIME, but there is no means of condensing the data as such. Taken from the Second Lab, section 1-2:
...
2.  Identify **one or two variables** that you think should be:
    ...
    -   **Unique Key**: There exists no Unique Key or any small consistent combination of keys that can indicate uniqueness between each row. 
...
- Under the assumption that there is no data dictionary like this project, I will like to assume that there is no one set way of doing data cleaning, and therefore for me to say there is an exact way to clean data would be quite rambuctious of me. 
