...
```{r}
bids <- select(.data=mutate(.data=bids, PRICE_clean=PRICE), -PRICE)
idx <- which(substr(bids$PRICE_clean, 1, 1)=="O")
bids$PRICE_clean[idx] <- sub("^O", "0", bids$PRICE_clean[idx])
bids <- mutate(.data=bids_new, PRICE_clean=as.numeric(PRICE_clean))
bids <- mutate(.data=bids_new, PRICE_clean=if_else(PRICE_clean==-999, NA, PRICE_clean))
rm(idx)
```
