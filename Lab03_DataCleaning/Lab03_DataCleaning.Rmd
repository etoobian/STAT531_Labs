...
1.  In **2–4 sentences**, describe **what each row** in this dataset represents. For each row, there exists three unique identifiers that can be utilized to identify individual rows: AUCTION_ID, PUBLISHER_ID and RESPONSE_TIME. These rows can then be further grouped into blocks by AUCTION_ID to classify by individuals auctions being taken place. Each row of this subset of rows represent an individual bid with a given PRICE (in cost per mille) that is paired with meta data that can be utilized for further analysis.
2.  Identify **one or two variables** that you think should be:
    ...
    -   **Unique Key**: AUCTION_ID; though this is one of the three keys that are utilized to delineate unique rows, without an AUCTION_ID, there exists no way to classify what anythig is being bid on.
3.  Pick **two variables** and, for each, assign at least one **data quality dimension** (e.g., completeness, validity, consistency, uniqueness, cross-field consistency).
    ...
    -   AUCTION_ID must be unique within their own groupings to ensure that all other columns stay relevant and usable.
...
```{r}
bids <- mutate(.data=bids, PRICE_clean=PRICE)
idx <- which(substr(bids$PRICE_clean, 1, 1)=="O")
bids$PRICE_clean[idx] <- sub("^O", "0", bids$PRICE_clean[idx])
bids <- mutate(.data=bids, PRICE_clean=as.numeric(PRICE_clean))
rm(idx)
bids <- mutate(.data=bids, PRICE_clean=if_else(PRICE_clean==-999, NA, PRICE_clean))
bids <- mutate(.data=bids, PRICE_clean=if_else(PRICE_clean < 0, -PRICE_clean, PRICE_clean))
```
