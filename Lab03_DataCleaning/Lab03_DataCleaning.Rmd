...
We first used dplyr::count() to inspect all distinct DEVICE_GEO_REGION values and identify inconsistent or messy forms of the Oregon region code. After trimming whitespace, converting to uppercase, and removing punctuation, we standardized several known variants of the Oregon region code. Specifically, we mapped the values "OR", "OREGON", and "XOR" to the standard code "OR", and any remaining values that did not match these patterns were treated as invalid and set to NA.
...
bids <- bids %>%
  dplyr::mutate(
    ...
    # 2) Convert ZIP to character
    zip_char = as.character(zip_trim),

    # 3) Check sentinel BEFORE padding
    is_sentinel = zip_char %in% c("-999", "9999", "00000"),

    # 4) Replace sentinel with NA
    zip_no_sentinel = dplyr::if_else(is_sentinel, NA_character_, zip_char),

    # 5) Now pad ZIPs after removing sentinel values
    zip_padded = stringr::str_pad(zip_no_sentinel, width = 5, side = "left", pad = "0"),

    # 6) Check valid Oregon ZIPs
    is_valid_oregon_zip = !is.na(zip_padded) &
                          stringr::str_detect(zip_padded, "^97[0-9]{3}$"),
...
bids <- bids %>%
  mutate(
   # 1) Remove the exact prefix "RESPONSE_TIME:" from the beginning of the string.
