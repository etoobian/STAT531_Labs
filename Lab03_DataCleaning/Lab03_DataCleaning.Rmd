...
bids <- bids %>%
  ...
  # Standardize all Oregon variants to "OR"
    DEVICE_GEO_REGION_clean = dplyr::case_when(
      region_clean %in% c("OR", "ORE", "OREG", "OREGON") ~ "OR",
      TRUE ~ NA_character_
    )
  )
...
We used dplyr::count() to inspect all distinct DEVICE_GEO_REGION values and identify inconsistent Oregon codes. After trimming whitespace and converting all values to uppercase, we standardized the valid variants "OR" and "OREGON" to the clean code "OR", while treating "XOR" and any other unexpected patterns as invalid and setting them to NA.
...
bids_zip_check <- bids %>%
  mutate(
    # 1) Normalize ZIP: character + trim spaces
    zip_raw   = as.character(DEVICE_GEO_ZIP),
...
Our rule defines a “valid” ZIP code as a clean 5-digit Oregon ZIP between 97001 and 97920.This rule is reasonable because the dataset only contains bids from Oregon, so any value outside this range cannot represent a real Oregon location.  After examining the raw ZIP codes to identify common problems, we looked for suspicious patterns like sentinel values (-999), negative signs, non-digit characters, and incorrect lengths.  After extracting the first five digits and checking whether they formed a valid Oregon ZIP, we kept only those that passed all checks and set everything else to NA.  This ensures that our cleaned ZIP codes are realistic, consistent, and aligned with the geographic scope of the data.
