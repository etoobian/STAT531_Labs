...
```{r initial-orientation-1, message=FALSE, 'hold'=TRUE}
# Dimensions
n_rows <- nrow(bids)
n_cols <- ncol(bids)
...
```{r initial-orientation-3, message=FALSE}
# Column classes
(col_types <- sapply(bids, class))

# Unique values in each column <- sapply(bids, function(x), length(unique(x))
```

  
  3.
  
```{r initial-orientation4, message=FALSE}
